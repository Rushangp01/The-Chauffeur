...
const ConfirmRidePopUp = (props) => {
    ...
    const submitHander = async (e) => {
        ...
        if (response.status === 200) {
            ...
            navigate('/captain-riding', { state: { rideData: props.ride } }) // <-- FIXED!
        }
        else {
            console.log('Error starting ride:', response.data.message)
        }
    }    
  return (
    <div>
    <h5 className="p-1 text-center w-[93%] absolute top-0" onClick={()=>{
          ...
          <div className='mt-6 w-full'>
            <form onSubmit={submitHander} className='flex flex-col gap-3'>
                <input value={otp} onChange= {(e) =>setOtp(e.target.value)}className='px-6 py-4 font-mono bg-[#eee] px-12 py-2 text-base rounded-lg w-full mt-3' type='text' placeholder='Enter OTP'/>
                ...
                <button className='w-full mt-5 text-lg flex justify-center bg-green-600 text-white font-semibold p-3 rounded-lg'>Confirm</button>
                      ...
                      <button onClick={() => {
                       props.setConfirmRidePopupPanel(false)
                       props.setRidePopupPanel(false)
                     }} className="w-full mt-4 bg-red-500 text-white  font-semibold p-2 rounded-lg">Cancel</button>
              </form>
          </div>
          </div>
    </div>
